...
def create_gui_P230(create_login_ui, create_gui_P1, create_gui_P4):
    ...
    Copyright = ttk.Label(root_P230, text="Powered by ITM Semiconductor Vietnam Company Limited - IT Team. Copyright © 2024 all rights reserved.", anchor="center")
    ...
    def check_heater_id(event=None):
        ...
        if result[0] == 'PASS':
            ...
            log_message(f"Check HEATER_ID: {heater_id} in Model P2 3.0 result {result[0]} at {result_time}!.\n")
        else:
            ...
            if result[0] == 'FAIL':
                ...
                display_result('NG')
                name.config(text=f"{heater_id}")
                log_message(f"Check HEATER_ID: {heater_id} in Model P2 3.0 does not exist!.\n")
        
        entry.delete(0, tk.END)

    root_P230.title(f"P2HB 3.0 Heater ID Result Checker Version {get_current_version()}")
    root_P230.geometry("1065x800")
    # root.resizable(False, False)
    font_entry = font.Font(family="Helvetica", size=16, weight="bold")

    label = tk.Label(root_P230, text="Enter HEATER_ID:")
    label.pack(pady=10)

    entry = tk.Entry(root_P230, width=40, font=font_entry)
    entry.pack(pady=10)

    entry.bind('<Return>', check_heater_id)

    result_label = Label(root_P230)
    result_label.pack(pady=10)

    name = tk.Label(root_P230, text='HEATER_ID', font=("Arial Bold", 26))
    name.pack(pady=10)
