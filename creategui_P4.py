            ...
            with conn.cursor() as cursor:
                query = """
                    ...
                    FROM (
                        ...
                        FROM ITMV_KTNG_DB.dbo.AFA_P4_RESISTANCE_TEST_HISTORY
                        WHERE HEATER_ID = ?
                    ) A 
                    WHERE A.RNK = 1
                """
...
def execute_query(heater_id):
    ...
    query = """
            ...
            FROM (
                ...
                FROM MIGHTY.ADC_P4_RESISTANCE_TEST_HISTORY aprth 
...
def create_gui_P4(create_login_ui, create_gui_P1, create_gui_P230):
    ...
    root_P4 = tk.Tk()
    icon_mesconnect = ttk.Label(root_P4, text="")
    icon_mesconnect.place(x=10, y=795, width=25, height=25)
    name_mesconnect = ttk.Label(root_P4, text="", anchor="w")
    name_mesconnect.place(x=40, y=795, width=200, height=25)
    Copyright = ttk.Label(root_P4, text="Powered by ITM Semiconductor Vietnam Company Limited - IT Team. Copyright © 2024 all rights reserved.", anchor="center")
    ...
    def check_heater_id(event=None):
        ...
        if result[0] == 'PASS':
            ...
            log_message(f"Check HEATER_ID: {heater_id} in Model P4 result {result[0]} at {result_time}!.\n")
        else:
            ...
            if result[0] == 'FAIL':
                ...
                display_result('NG')
                name.config(text=f"{heater_id}")
                log_message(f"Check HEATER_ID: {heater_id} in Model P4 does not exist!.\n")
        
        entry.delete(0, tk.END)

    root_P4.title(f"P4 Heater ID Result Checker Version {get_current_version()}")
    root_P4.geometry("1065x800")
    # root.resizable(False, False)
    font_entry = font.Font(family="Helvetica", size=16, weight="bold")

    label = tk.Label(root_P4, text="Enter HEATER_ID:")
    label.pack(pady=10)

    entry = tk.Entry(root_P4, width=40, font=font_entry)
    entry.pack(pady=10)

    entry.bind('<Return>', check_heater_id)

    result_label = Label(root_P4)
    result_label.pack(pady=10)

    name = tk.Label(root_P4, text='HEATER_ID', font=("Arial Bold", 26))
    name.pack(pady=10)
